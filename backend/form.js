...
app.get('/', (req, res) => {
  res.send('<h1 style="text-align: center;">Welcome to Our Activities Form backend!</h1>');
});
...
app.post('/record', (req, res) => {
    ...
    const record = {
      eng_first_name: data.eng_first_name,
...
app.get('/getFaculty', (req, res) => {
  fs.readFile('databases/faculty.json', 'utf8', (err, data) => {
    if (err) {
    ...
    try {
      ...
      console.error('Error parsing faculty.json:', err);
      return res.status(500).json({ error: 'Internal server error' });
    }
  });
});
